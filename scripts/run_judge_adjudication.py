...
from lssa.judging.nvidia import NvidiaGuardJudge, NvidiaJudgeConfig
...
def main(argv: list[str] | None = None) -> int:
    ...
    config = NvidiaJudgeConfig.from_env()
    plan_path = _write_plan(args.plan_dir, root, records, config, args)
    if not args.allow_judge_network:
        print(
            f"dry-run judge_provider=nvidia_nim model={config.model} "
    ...
    for record in records:
        ...
        try:
            ...
            print(
                f"judge_provider=nvidia_nim status=error prompt_id={record.prompt_id} "
                f"error_type={type(exc).__name__}",
                file=sys.stderr,
            )
            failures += 1
            continue
        result_path = output_dir / f"judge-{uuid4().hex}.json"
        result_path.write_text(
            json.dumps(result.to_redacted_dict(), indent=2, sort_keys=True),
            encoding="utf-8",
        )
        print(
            f"judge_provider=nvidia_nim status=ok prompt_id={record.prompt_id} "
